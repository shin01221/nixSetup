...
in
{
  home.activation.ensureWlrWhichKeyConfig = lib.hm.dag.entryAfter ["writeBoundary"] (
    let
      niriYaml = pkgs.writeText "wlr-which-key-niri-yaml" (
        readFile ../config/wlr-which-key/niri.yaml
      );
      colorsJson = pkgs.writeText "wlr-which-key-colors-json" (
        readFile ../config/wlr-which-key/colors.json
      );
    in
    ''
      ${hashUpdate "wlr-which-key/niri.yaml" niriYaml}
      ${hashUpdate "wlr-which-key/colors.json" colorsJson}
    ''
  );
}
