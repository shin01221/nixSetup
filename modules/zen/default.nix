...
let
  cfg = config.workstation.zen;
in {
  options.workstation.zen.enable = lib.mkEnableOption "Zen browser configuration";
      ...
      (
        { config, ... }:
        {
          ...
          programs.zen-browser = {
            enable = true;
            policies = import ./policies.nix { inherit lib config pkgs; };
            languagePacks = [
              "en-GB"
              "en-US"
            ];
